...
from crm.models import Company, Contract, ShareHolder, TaxBureau
...
class CompanyModelAdmin(AdminViewPermissionModelAdmin):
    ...
    list_filter = ('status', 'ic_status', 'license_status', 'type', 'salesman',
                   'rating', 'industry', 'has_custom_info',
                   'has_customer_files', 'taxpayer_type', 'scale_size')
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        if len(request.GET) == 0 and '?' not in referrer:
            return redirect("{url}?{get_parms}".format(
                url=request.path, get_parms=get_param))
    ...
    def get_form(self, request, obj=None, **kwargs):
        form = super(CompanyModelAdmin, self).get_form(request, obj, **kwargs)
        if 'special_taxes' in form.base_fields:
            form.base_fields['special_taxes'].widget.attrs[
                'style'] = 'width: 610px;'
